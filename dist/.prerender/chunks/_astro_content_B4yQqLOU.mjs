...
import { c as renderElement, g as generateCspDigest, s as spreadAttributes, u as unescapeHTML, a as renderTemplate, d as removeBase, i as isRemotePath, A as AstroError, U as UnknownContentCollectionError, R as RenderUndefinedEntryError, p as prependForwardSlash, e as createHeadAndContent, r as renderComponent } from './prerender_dkmDDFL_.mjs';
import { c as createComponent } from './Layout_BWxZLDmF.mjs';
import 'clsx';
import 'piccolore';
...
class ImmutableDataStore {
  ...
  static async fromModule() {
    try {
      const data = await import('./_astro_data-layer-content_Cide9H1P.mjs');
...
function createGetCollection({
  ...
  return async function getCollection(collection, filter) {
    ...
    if (store.hasCollection(collection)) {
      const { default: imageAssetMap } = await import('./content-assets_uO_LFzdT.mjs');
      const result = [];
...
async function updateImageReferencesInBody(html, fileName) {
  ...
  const { getImage } = await import('./_virtual_astro_get-image_BF5W5DIl.mjs').then(n => n._);
...
async function renderEntry(entry) {
  ...
  if (entry.deferredRender) {
    try {
      const { default: contentModules } = await import('./content-modules_BWXpjLwY.mjs');
